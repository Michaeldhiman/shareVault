          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-2.5 group" aria-label="SecureVault Home">
              <div className="relative w-8 h-8 flex items-center justify-center">
                ...
                <Shield size={16} className="relative z-10 text-emerald-400 group-hover:text-emerald-300 transition-colors duration-300" strokeWidth={2.5} />
            ...
            <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
              {NAV_LINKS.map((link) => (
                <button
                  ...
                  className="px-4 py-2 text-sm font-display font-medium text-white/50 hover:text-white/90 rounded-lg hover:bg-white/5 transition-all duration-200 cursor-pointer"
                >
                  {link.label}
                </button>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-3">
              <Link to="/login" className="px-4 py-2 text-sm font-display font-medium text-white/60 hover:text-white/90 transition-colors duration-200">
                Sign in
              </Link>
              <motion.div
                ref={ctaMagnetic.ref}
                ...
                animate={{ x: ctaMagnetic.pos.x, y: ctaMagnetic.pos.y }}
                transition={{ type: 'spring', stiffness: 300, damping: 20 }}
              >
                <Link to="/register" className="btn-primary text-sm px-5 py-2.5" aria-label="Get started with SecureVault">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            style={{ background: 'rgba(2,2,3,0.97)', backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)' }}
            role="dialog"
            aria-modal="true"
          >
            ...
            <div className="flex-1 flex flex-col pt-24 pb-8 px-6">
              <nav className="flex flex-col gap-1 mb-8">
                {NAV_LINKS.map((link, i) => (
                  <motion.button
                    ...
                    onClick={() => handleNavClick(link.href)}
                    className="flex items-center justify-between w-full px-4 py-4 text-lg font-display font-medium text-white/70 hover:text-white rounded-xl hover:bg-white/5 transition-all duration-200 cursor-pointer group"
                  >
                    {link.label}
              ...
              >
                <Link to="/register" onClick={() => setMobileOpen(false)} className="btn-primary w-full justify-center text-base py-4">
                  <Shield size={16} strokeWidth={2.5} /> Get started free
                </Link>
                <Link to="/login" onClick={() => setMobileOpen(false)} className="btn-secondary w-full justify-center text-base py-4">
              ...
              <motion.div
                ...
                className="mt-auto flex items-center justify-center gap-4 text-xs text-white/30"
              >
